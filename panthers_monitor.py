#!/usr/bin/env python3
"""
Panthers Monitor — Raspberry Pi Zero WH
Monitors the NHL API for Panthers goals and writes events to the shared queue.
...
def get_todays_panthers_game() -> dict | None:
    try:
    ...
    for day in data.get("gameWeek", []):
        for game in day.get("games", []):
            ...
            if PANTHERS_TEAM_ID in (home, away):
                return {
                    "game_id": game["id"],
...
def get_game_data(game_id: int) -> dict | None:
    ...
    if home_team.get("abbrev") == PANTHERS_TEAM_ID:
        panthers_team = home_team
    elif away_team.get("abbrev") == PANTHERS_TEAM_ID:
        panthers_team = away_team
    else:
        return None

    score = panthers_team.get("score", 0)
    ...
    if period_type == "SO":
        ...
        for attempt in shootout_data:
            if not isinstance(attempt, dict):
                continue
            if (attempt.get("teamAbbrev") == PANTHERS_TEAM_ID and
...
def main():
    print(f"[START] Panthers monitor running...")
    ...
    try:
        while True:
            ...
            game = get_todays_panthers_game()

            if game is None:
                print(f"[{now}] No Panthers game today. Checking again in {IDLE_INTERVAL}s.")
                last_goal_count = None
                last_so_goals   = None
                active_game_id  = None
            ...
            if state in ("LIVE", "CRIT", "FINAL", "OFF"):
                ...
                if period_type == "SO":
                    if last_so_goals is None:
                        ...
                        print(f"[{now}] Shootout started! Panthers SO goals: {so_goals} "
                              f"(baseline set)")
                    elif so_goals > last_so_goals:
                        new_so_goals = so_goals - last_so_goals
                        print(f"[{now}] PANTHERS SHOOTOUT GOAL! (+{new_so_goals})")
                        for _ in range(new_so_goals):
                            queue_event("panthers_shootout")
                            time.sleep(1)
                        last_so_goals = so_goals
                    else:
                        print(f"[{now}] {matchup} | SO | Panthers SO goals: {so_goals}")
                ...
                # ── Regular / OT ───────────────────────────────────────────────
                    ...
                    if last_goal_count is None:
                        ...
                        print(f"[{now}] Game live ({matchup}) | Period: {period_type} | "
                              f"Panthers goals: {current_goals} (baseline set)")

                    elif current_goals > last_goal_count:
                        new_goals = current_goals - last_goal_count
                        print(f"[{now}] PANTHERS SCORED! (+{new_goals}) | Total: {current_goals}")
                        for _ in range(new_goals):
                            queue_event("panthers_goal")
                            time.sleep(1)
                        last_goal_count = current_goals

                    else:
                        print(f"[{now}] {matchup} | {period_type} | State={state} | "
                              f"Panthers goals: {current_goals}")

                if state in ("FINAL", "OFF"):
                    print(f"[{now}] Game over. Final Panthers goals: {current_goals}. "
